          ...
          <div style="max-width: 600px; animation: fadeInUp 0.6s ease;">
            ...
            ${!isNewPlayer ? `
              <div style="display: flex; gap: var(--space-lg); justify-content: center; margin-bottom: var(--space-xl);">
                ...
                <div style="text-align: center;">
                  <div style="font-size: var(--text-3xl); font-weight: 800; color: var(--color-success);">${GameState.get('progress.calculus.completed').length + GameState.get('progress.python.completed').length + GameState.get('progress.algebra.completed').length + GameState.get('progress.trigonometry.completed').length}</div>
